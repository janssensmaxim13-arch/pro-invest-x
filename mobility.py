...
def render(username=None):
    ...
    tabs = st.tabs(["Travel Packages", "Shuttle", "VIP Transfer"])
    ...
    with tabs[1]:
        ...
        pickup = st.selectbox("Pickup", MOROCCAN_CITIES)
        dropoff = st.selectbox("Dropoff", MOROCCAN_CITIES)
    ...
    with tabs[2]:
        ...
        if st.button("Book VIP"):
            st.success(f"VIP booked! Ref: VIP-{random.randint(1000,9999)}")
```

### Stap 4: Save
`Ctrl+S` in Notepad

### Stap 5: Start opnieuw
```
